...
-Bank coins
-Transfer coins to spare change
-Go to bank view
*/
...
public class ViewWallet extends AppCompatActivity {
    ...
    private ArrayList<String> coinz = new ArrayList<>();
    private ArrayList<String> selectedcoinz = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //initialize firebase variables and get current user
        mAuth = FirebaseAuth.getInstance();
        curruser = mAuth.getCurrentUser().getEmail();

        //check date to update coins banked number
        datechecker();
              ...
              new OnSuccessListener<QuerySnapshot>() {
                  ...
                  public void onSuccess(@Nullable QuerySnapshot queryDocumentSnapshots) {
                      for (int i = 0; i < queryDocumentSnapshots.size(); i++) {
                          ...
                          //store with id so we know which coins to manipulate later
                          //on along with their values
                          coinsandid.put(id, value + " " + curr);
                      }

                      //set up list view with this list and the templates we created separately
                      arrayAdapter = new ArrayAdapter(ViewWallet.this,
                                                       R.layout.viewcoinzrow, R.id.template, coinz);
    ...
    public void bankcoins(View view) {
        ...
        if (selectedcoinz.size() > 0 && selectedcoinz.size() <= 25) {
              ...
              .addOnSuccessListener(
                 new OnSuccessListener<DocumentSnapshot>() {
                     ...
                     public void onSuccess(@android.support.annotation.Nullable DocumentSnapshot documentSnapshot) {
                         ...
                         //make sure that they haven't banked 25 or that them banking the set amount doesn't
                         ...
                         if (currtotalbanked < 25 && currtotalbanked + selectedcoinz.size() <= 25) {
                            ...
                            for (int i = 0; i < selectedcoinz.size(); i++) {
                                calcgold = calcgold +
                                        (new ConverterandDialogs()).currconverter(currentrates, selectedcoinz.get(i));
                            ...
                            for (Map.Entry m : coinsandid.entrySet()) {
                                if (selectedcoinz.contains(m.getValue())) {
                                    ...
                                    db.collection("users").document(curruser)
                                            .collection("wallet").document(key).delete();
                                }
                            }
                        ...
                        new ConverterandDialogs().OKdialog("You can only bank 25 in a day!",
                                       "Banking limit reached",ViewWallet.this).show();
                    }
                    //update today banked number and gold in users account
                    db.collection("users").document(curruser)
                                           .update("Today Banked", "" + calctodaybanked);

                    db.collection("users").document(curruser).update("Gold", calcgold);

                    //reset values in case user wants to bank more
                    calctodaybanked = 0;
                    calcgold = 0.0;
                }
            });
            ...
            if (selectedcoinz.size() <1)
                new ConverterandDialogs().OKdialog("Please select coins to bank!",
                                           "No coins selected",ViewWallet.this).show();
            if (selectedcoinz.size() > 25)
                new ConverterandDialogs().OKdialog("You can not bank more than 25 coins" +
                                " in  a day!","Limit Exceeded",ViewWallet.this).show();
            }
    }
    //========================================END BANKING COINS=====================================

    //========================================TRANSFERRING TO SPARE=================================
    //move coins to spare change
    public void transfertosparechange(View view) {
        //make sure coins are selected
            ...
            db.collection("users").document(curruser).get()
               .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                   @Override
                    public void onSuccess(@Nullable DocumentSnapshot documentSnapshot) {
                        ...
                        for (Map.Entry m : coinsandid.entrySet()) {
                            if (selectedcoinz.contains(m.getValue())) {
                                ...
                                //get the value from the hash map to get currency and value of the coin
                                //the value is store like "5.3493824343 DOLR"
                                String value = String.valueOf(m.getValue());

                                //the value is store like "5.3493824343 DOLR" hence we get the
                                //currency by taking the last 4 characters and the value by taking
                                //all the characters till the space
                                String currency = value.substring(value.length() - 4, value.length());
                                String valueofcoin = value.substring(0, value.indexOf(" "));

                                //setup up the coin object to transfer
                                Coin coin = new Coin(key, valueofcoin, currency);

                                //add to spare change
                                db.collection("users").document(curruser)
                                        .collection("spare change").document(key).set(coin);
                    ...
                    new ConverterandDialogs().OKdialog("Coins Moved to Spare Change!",
                                                   "Success", ViewWallet.this).show();
                }
            });
        }else {
            new ConverterandDialogs().OKdialog("Please select coins to transfer!",
                                         "No Coins Selected", ViewWallet.this).show();
        }
    }
    //=====================================END TRANSFERRING TO SPARE================================

    //check if date has changed
    public void datechecker() {
        db.collection("users").document(curruser).get().addOnSuccessListener(
          new OnSuccessListener<DocumentSnapshot>() {
              @Override
                public void onSuccess(@android.support.annotation.Nullable DocumentSnapshot documentSnapshot) {
                    //get date from the users account and compare to current date
